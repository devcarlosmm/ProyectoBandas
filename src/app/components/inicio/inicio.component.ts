...
export class InicioComponent implements OnInit {
  ...
  eliminarBanda(idBanda: number, nombre_banda: string): void {
    const borrarBanda = confirm(
      `Estas seguro de querer borrar a '${nombre_banda}'`
    );
    if (borrarBanda) {
  ...
  cortarHistoria(pBandas) {
    ...
    for (let i = 0; i < nHistorias; i++) {
      ...
      let segundaHistoria = pBandas[i].historia.substring(
        300,
        pBandas[i].historia.length
      );
